...
files = os.listdir(r'D:\Рабочая\САМОТЕЧНОЕ ОБОРУДОВАНИЕ\ЗАДВИЖКИ\УЗР-300_\DXF!')  # ссылка на папку с DXF
files_no_dxf = []

# наполняем список files_no_dxf названиями файлов dxf без расширений
for i in files:
    if i[-4:].lower() == '.dxf':
        files_no_dxf.append(i[:-4])
wb = load_workbook(r'D:\Рабочая\САМОТЕЧНОЕ ОБОРУДОВАНИЕ\ЗАДВИЖКИ\УЗР-300_\Заявка УЗР-300 №хх от 00.00.2021.xlsx')  # ссылка на файл excel
ws = wb.active
